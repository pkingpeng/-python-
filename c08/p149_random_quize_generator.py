...
假如你是一位地理老师，班上有 35 名学生，你希望进行美国各州首府的一个小测验。不妙的是，班里有几个坏蛋，你无法确信学生不会作弊。
...
for quizNum in range(35):
    quizFile = open('../resource/capitalsquize/%s_question.txt' % (quizNum + 1), 'w')
    answerKeyFile = open('../resource/capitalsquize/%s_answer.txt' % (quizNum + 1), 'w')
    ...
    for questionNum in range(50):
        ...
        del wrongAnswers[wrongAnswers.index(correntAnswer)]
        wrongAnswers = random.sample(wrongAnswers, 3)
        answerOptions = wrongAnswers + [correntAnswer]
        random.shuffle(answerOptions)

        quizFile.write('%s. What is the captital of %s?\n' % (questionNum + 1, states[questionNum]))
